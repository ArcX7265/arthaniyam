...
class RefundService:
    ...
    def evaluate(self, request: RefundEvaluationRequest) -> RefundEvaluationResult:
        with self.guard.repository.transaction():
            return self.evaluate_in_transaction(request)

    def evaluate_in_transaction(self, request: RefundEvaluationRequest) -> RefundEvaluationResult:
        """Internal synchronous entry point for a caller-owned repository transaction."""
        if not getattr(self.guard.repository._transactions, "connection", None):
            raise RuntimeError("refund admission requires an active transaction")
